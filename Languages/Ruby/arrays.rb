...
puts array[3] # outputs 4 (as index values start at 0)
...
a.fill(obj, start, length) # Fill array a with obj from start to length
a.insert(index, obj) # Insert given obj before element at given index of array a
...
a.delete_at(index) # Delete element at index of array a, returning that element, or nil if index is out of range
...
a.reject! # Equivalent to a.delete_if, deleting elements from a for which the block evaluates to true, but returns nil if no changes were made
...
a1.initialize_copy(a2) # Replace contents of a1 with contents of a2, truncating or expanding if necessary
